...
class BinaryCrossEntropyLoss(Module):
    ...
    def __call__(self, logits: Value, targets: np.ndarray) -> Value:
        ...
        probs = logits.sigmoid() # Placeholder
        ...
        loss_elements = loss_elements = -(targets_val * probs_stable.log() + (Value(1) - targets_val) * (Value(1) - probs_stable).log())
...
class CrossEntropyLoss(Module):
    ...
    def __call__(self, input_logits: Value, target: np.ndarray) -> Value:
        ...
        #print("TODO: Implement numerically stable LogSoftmax using Value ops in CE Loss.")
             # Placeholder, shape (batch_size, n_classes)
        max_logits = np.max(input_logits.data , axis = 1 , keepdims = True)
        stable_logits = input_logits - Value(max_logits)
        exp_stable_logits = stable_logits.exp()
        sum_exp_logits = exp_stable_logits.sum(axis = 1 , keepdims = True)
        log_sum_logits = sum_exp_logits.log()
        log_probs = stable_logits - log_sum_logits
        ...
        nll_per_sample = y_one_hot * log_probs # Placeholder, shape (batch_size,)
        nll_per_sample = -nll_per_sample
        nll_per_sample = nll_per_sample.sum(axis = 1)
