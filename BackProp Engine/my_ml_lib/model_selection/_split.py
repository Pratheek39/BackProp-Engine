...
def train_test_split(X, y, test_size=0.2, shuffle=True, random_state=None):
    ...
    if not (0.0 <= test_size <= 1.0):
        raise ValueError("Invalid test size argument")
...
def train_test_val_split(X, y,
    ...
    if not(0.0 <= train_size <= 1.0 and 0.0 <= test_size <= 1.0 and 0.0 <= val_size <= 1.0 and np.isclose(test_size + train_size + val_size , 1) ):
        raise ValueError("Invalid test, train or val sizes")
