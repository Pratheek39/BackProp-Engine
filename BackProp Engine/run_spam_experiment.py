...
def find_best_alpha(X_train_cv, y_train_cv, alphas, n_splits, random_state):
        ...
        for alpha in alphas:
          print(f"Finding best alpha for alpha = {alpha}")
          curr_score = 0
          generator = kfold.split(X_train_cv , y_train_cv)
          for train_indices , test_indices in generator:
                X_train_k_fold = X_train_cv[train_indices]
                y_train_k_fold = y_train_cv[train_indices]
                X_test_k_fold = X_train_cv[test_indices]
                y_test_k_fold = y_train_cv[test_indices]

                model = LogisticRegression(alpha = alpha)
                model.fit(X_train_k_fold , y_train_k_fold)
                curr_score += model.score(X_test_k_fold , y_test_k_fold)
...
train_error_raw = 1 - model_raw.score(X_train , y_train) # Placeholder
test_error_raw = 1 - model_raw.score(X_test , y_test) # Placeholder
...
X_train_std = scale_transform.transform(X_train)# Placeholder
X_test_std = scale_transform.transform(X_test)  # Placeholder
...
train_error_std = 1 - model_std.score(X_train_std , y_train) # Placeholder
test_error_std = 1 - model_std.score(X_test_std , y_test) # Placeholder
...
train_error_no_reg = 1 - model_no_reg.score(X_train, y_train)
test_error_no_reg = 1 - model_no_reg.score(X_test , y_test)
print(f"Without Regularzation, train error is : {train_error_no_reg}  and  the test error is {test_error_no_reg}")